...
import uuid
from flask import Flask, request, jsonify, render_template, session # Added session here
import hashlib
import subprocess
import requests
...
SUPERVISOR_PROMPT = "You are the Train Garden Supervisor. Route the user's request effectively."
COACH_PROMPT = "You are the Train Garden Career Coach. Provide supportive and actionable advice."
...
def recall_relevant_context(user_input, user_id):
    # 1. VALIDATION FIX: Ensure user_input is a string and not empty
    if not user_input or not isinstance(user_input, str):
        print(f"Warning: Invalid user_input type: {type(user_input)}")
        return ""

    try:
        # 2. CHROMA QUERY
        results = vector_memory.query(
            query_texts=[user_input],  # ChromaDB expects a list of strings
            n_results=3,
            where={"user_id": user_id}
        )
...
class CareerOrchestrator:
    def __init__(self, user_id):
        ...
        self.history = self._get_recent_history()

    def _get_recent_history(self):
        # Pulls the last 3 interactions to stay 'intuitive'
        mem_file = os.path.join(MEMORY_DIR, f"{self.user_id}.json")
        if os.path.exists(mem_file):
            with open(mem_file, 'r') as f:
                data = json.load(f)
                return str(data.get('history', []))[:1000] # Limit context size
...
def process(self, user_input):
    ...
    # This line was failing because 'session' wasn't imported
    self.user_id = session.get('user_id', 'anonymous') 
    
    past_context = recall_relevant_context(user_input, self.user_id)
    routing_prompt = f"{SUPERVISOR_PROMPT}\nHistory: {self.history}\nInput: {user_input}"[cite: 4]
    
    response_text = query_ai(f"{COACH_PROMPT}\nContext: {past_context}\nUser: {user_input}")[cite: 4]

    self.history.append({"user": user_input, "ai": coach_voice})
        
        # PERSIST TO MEMORY VAULT (ChromaDB example)
        # We store the interaction so it survives a page refresh
        vector_memory.add(
            documents=[f"User asked about {user_input}. AI suggested: {coach_voice}"],
            metadatas=[{"type": "chat_history", "user_id": self.user_id}],
            ids=[f"chat_{uuid.uuid4()}"]
        )

        return {
            "message": final_message,
            "resources": research_data.get("resources", []),
            "history_item": {"title": user_input[:20] + "...", "id": "unique_id_123"}
        }
    
    return {"message": response_text}[cite: 4]
...
class Orchestrator:
    ...
    def process(self, user_input):
        ...
        # --- AGENT 1: THE RESEARCHER (Open Source Intelligence) ---
        # Forces a lookup of actual industry standards
        research_prompt = f"""
        Determine current job market requirements for Sustainability/ESG roles related to: {user_input}.
        Identify 3 skills, 3 gaps, and 2 verified URLs from (GRI, IFRS, Coursera, or LinkedIn Learning).
        Return ONLY JSON: 
        {{"matched": [], "gaps": [], "resources": [{{"url":"", "desc":"", "overview":""}}]}}
        """
        raw_research = query_ai(research_prompt)

        # Robust JSON Extraction
        try:
            json_match = re.search(r'\{.*\}', raw_research, re.DOTALL)
            research_data = json.loads(json_match.group(0)) if json_match else {}
        except:
            research_data = {"matched": ["Analyzing..."], "gaps": [], "resources": []}

        # --- AGENT 2: THE STRATEGIST (Step-by-Step Roadmap) ---
        # This agent builds the execution plan to fill the gaps
        strategy_prompt = f"""
        User Input: {user_input}
        Gaps Identified: {research_data.get('gaps')}
        Provide a 3-step career roadmap to transition into a Sustainability Lead role.
        Be specific about frameworks (e.g., Carbon Accounting, GHG Protocol).
        """
        step_by_step_plan = query_ai(strategy_prompt)

        # --- AGENT 3: THE MEMORY KEEPER (Vault Persistence) ---
        # This stores the roadmap in the history for the sidebar
        memory_item = {
            "id": str(uuid.uuid4()),
            "title": f"Sustainability Plan: {user_input[:15]}...",
            "content": step_by_step_plan
        }
        # In a real app, you'd save this to ChromaDB or your SQL DB here
        # db.save(memory_item, user_id=self.user_id)

        # --- FINAL ASSEMBLY ---
        skills_payload = {
            "matched": research_data.get("matched", []),
            "gaps": research_data.get("gaps", [])
        }
        
        # Combine the voice and the technical data
        final_message = f"{step_by_step_plan}\n\nSKILLS_JSON: {json.dumps(skills_payload)}"

        return {
            "message": final_message,
            "resources": research_data.get("resources", []),
            "history_item": memory_item  # Sent to frontend to populate the Memory Vault
        }
...
def chat():
    data = request.json
    # Extract 'text' from the incoming JSON, default to empty string if missing
    user_input = data.get('text', '') 
    user_id = data.get('user_id')

    if not user_input:
        return jsonify({"message": "I didn't receive any text. How can I help?"}), 400

    try:
        # Now passing a guaranteed string to the orchestrator
        ai_response = orchestrator.process(user_input)
...
def upload_cv():
    try:
        ...
        # 2. Multi-Agent Prompting: Ask for both Report and JSON
        # This ensures the agent is proactive and provides the data for your right-hand tab
        prompt = f"""
        Analyze this CV against the Job Description: {job_desc}
        
        CV CONTENT: {cv_text[:2000]}
        
        Return the analysis in two parts:
        1. A detailed Markdown report for the user.
        2. A section at the end formatted EXACTLY as:
           SKILLS_JSON: {{"matched": ["Skill A"], "gaps": ["Skill B"]}}
        """
        
        analysis_raw = query_ai(prompt)
        
        # 3. Extract the JSON data for the visual sidebar
        skills_data = {"matched": [], "gaps": []}
        clean_message = analysis_raw
        ...
        if "SKILLS_JSON:" in analysis_raw:
            ...
            try:
                ...
                pass # Fallback if AI hallucinates bad JSON

        # 4. Create Workspace and save the HTML report
        folder_name = "gap-analysis-" + datetime.now().strftime("%H%M")
        workspace = os.path.join(SKILLS_DIR, folder_name)
        os.makedirs(workspace, exist_ok=True)
        
        report_html = markdown.markdown(clean_message, extensions=['tables'])
        with open(os.path.join(workspace, "report.html"), "w") as f:
            f.write(report_html)

        # 5. Return success with the visual data
        return jsonify({
            "success": True,
            "message": clean_message,
            "skills_data": skills_data, # Triggers your JS visualizer bars
            "path": f"/view-skill/{folder_name}/report",
            "suggestions": ["Find courses for these gaps", "Draft a cover letter"]
        })

    except Exception as e:
        # This block was missing, causing your SyntaxError
