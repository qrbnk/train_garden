...
"""
# Suggested Next Steps UI
    next_steps_html = f"""
    <div style="margin-top: 40px; padding: 20px; background: #f0f7ff; border-radius: 12px; border: 1px solid #cce3ff;">
        <h4 style="margin-top: 0; color: #0056b3;">💡 Next Steps for your {skill_name} journey:</h4>
        <ul style="font-size: 14px;">
            <li><strong>Refine:</strong> "AI, this report is too technical. Can you simplify the 'Architecture' section?"</li>
            <li><strong>Test:</strong> "Generate a 5-question quiz based on this whitepaper."</li>
            <li><strong>Apply:</strong> "How do I add this specific ESG framework to my CV?"</li>
        </ul>
        <div style="margin-top: 15px;">
            <input type="text" id="feedback-input" placeholder="Give feedback or ask a follow-up..." style="width: 80%; padding: 8px; border-radius: 6px; border: 1px solid #ccc;">
            <button onclick="sendFeedback()" style="padding: 8px 15px; background: #007bff; color: white; border: none; border-radius: 6px; cursor: pointer;">Send</button>
        </div>
    </div>
    <script>
        function sendFeedback() {{
            const val = document.getElementById('feedback-input').value;
            window.parent.postMessage({{ type: 'feedback', text: val }}, '*');
        }}
    </script>
    """
    # --- 4. TABLE GENERATION ---
